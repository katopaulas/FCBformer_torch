...
class TB(nn.Module):
    def __init__(self):
        ...
        EMBED_DIMS = [64, 128, 320, 512] #[32, 64, 160, 256]
        DEPTH = [3, 4, 18, 3] #[2, 2, 2, 2]
        FIRST_DIM = EMBED_DIMS[0]
        
        backbone = pvt_v2.PyramidVisionTransformerV2(
            patch_size=4,
            embed_dims=EMBED_DIMS,#embed_dims=[64, 128, 320, 512],
            num_heads=[1, 2, 5, 8],
            mlp_ratios=[8, 8, 4, 4],
            qkv_bias=True,
            norm_layer=partial(torch.nn.LayerNorm, eps=1e-6),
            depths= DEPTH,# [3, 4, 18, 3],
            sr_ratios=[8, 4, 2, 1],
        )

        checkpoint = torch.load("pvt_v2_b3.pth")
        #checkpoint = torch.load("pvt_v2_b0.pth")
        ...
        for i in range(4):
            self.LE.append(
                nn.Sequential(
                    RB(EMBED_DIMS[i], 64), RB(64, 64), nn.Upsample(size=88)
                )
            )
...
class FCBFormer(nn.Module):
    def __init__(self, size=256):
        ...
        #                                 nn.Conv2d(1, 8, kernel_size=3, stride=2, padding=1),  # Reduce to 128×128
        #                                 nn.GELU(),
        #                                 nn.Conv2d(8, 16, kernel_size=3, stride=2, padding=1),  # Reduce to 64×64
        #                                 nn.GELU(),
        #                                 nn.Conv2d(16, 32, kernel_size=3, stride=2, padding=1), # Reduce to 32×32
        #                                 nn.GELU(),
        #                                 nn.AdaptiveAvgPool2d(4),   # Further reduce to 4×4
        #                                 nn.Flatten(),              # 32×4×4 = 512 features
        #                                 nn.Linear(512, 64),
        #                                 nn.GELU(),
        #                                 nn.Dropout(0.3),
        #                                 nn.Linear(64, 1),
